...
class reporteAntiguedad_model extends CI_Model {
    ...
    public function getDatoseporteAdeudo($Cliente, $Region, $Estatus) {
        try {
            ...
            $this->db->select("T.ID, T.Cliente_ID AS IDCliente,"
                    ...
                    . "DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) AS DIF,
CASE WHEN DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) > 0
			AND  DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) < 16
	THEN T.Importe END AS 'UNO',
CASE WHEN DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) > 15
			AND  DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) < 31
	THEN T.Importe END AS 'DOS',
CASE WHEN DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) > 30
			AND  DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) < 46
	THEN T.Importe END AS 'TRES',
CASE WHEN DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) > 45
			AND  DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) < 61
	THEN T.Importe END AS 'CUATRO',
CASE WHEN DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) > 60
			AND  DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) < 121
	THEN T.Importe END AS 'CINCO',
CASE WHEN DATEDIFF(CURRENT_DATE(), date_format(str_to_date(T.FechaCreacion, '%d/%m/%Y'), '%Y-%m-%d')) > 120
	THEN T.Importe END AS 'SEIS', "
                    . "CASE
    WHEN T.EstatusTrabajo =  'Pedido'
	THEN '1 PEDIDO'
    WHEN T.EstatusTrabajo =  'Presupuesto'
